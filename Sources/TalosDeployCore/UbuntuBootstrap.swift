...
public final class UbuntuAutoinstallBuilder: @unchecked Sendable {
    ...
    public func renderUserData(spec: UbuntuInstallSpec, networkPlan: NetworkRebuildPlan) throws -> String {
        ...
        let shortHostname = shortHostName(firstNonEmpty(spec.hostname, networkPlan.hostname, fqdn, "tds-helper"))
    ...
    public func renderMetaData(spec: UbuntuInstallSpec, networkPlan: NetworkRebuildPlan) -> String {
        let hostname = shortHostName(firstNonEmpty(spec.hostname, networkPlan.hostname, spec.deviceID, "tds-helper"))
...
public struct HelperBootstrapRun: Codable, Equatable, Sendable {
...
public final class BootstrapHelperInstaller: @unchecked Sendable {
    ...
    public func buildRun(installSpec: UbuntuInstallSpec, oobURL: String = "", oobUsername: String = "") async throws -> HelperBootstrapRun {
...
public struct UbuntuCaptureLoader {
    ...
    public func load(from url: URL) throws -> NetworkPreservationSnapshot {
        ...
        do {
            ...
            return try loadLegacyCapture(from: url)
        }
    }
    ...
    private func resolveSnapshotURL(_ url: URL) throws -> URL {
        ...
        if fileManager.fileExists(atPath: url.path, isDirectory: &isDirectory), isDirectory.boolValue {
            ...
            let legacy = url.appending(path: "host-networking.json")
            if fileManager.fileExists(atPath: legacy.path) {
                return legacy
            }
            throw UbuntuBootstrapError.invalidCapture("No snapshot.json or host-networking.json found in \(url.path)")
        }
        return url
    }

    private func loadLegacyCapture(from url: URL) throws -> NetworkPreservationSnapshot {
        let directory = url.hasDirectoryPath ? url : url.deletingLastPathComponent()
        let hostNetworking = readLegacyText(from: directory.appending(path: "host-networking.txt"))
        let topology = readLegacyText(from: directory.appending(path: "network-topology.txt"))
        let device = (try? loadLegacyDevice(from: directory.appending(path: "core-device.json"))) ?? DiscoveredDevice(id: "", accountNumber: "", name: "")
        let summary = NetworkPreservationSummary(
            hostname: firstRegexGroup(#"---HOSTNAME---\n([^\n]+)"#, in: hostNetworking),
            osDescription: firstRegexGroup(#"Operating System:\s+([^\n]+)"#, in: hostNetworking),
            primaryIP: device.primaryIP,
            privateIP: device.privateIP,
            oobIP: device.oob?.address ?? "",
            observedInterfaces: lines(between: "---IP-BR-ADDR---", and: "---IP-DETAIL-LINK---", in: hostNetworking),
            observedRoutes: lines(between: "---ROUTE---", and: "---RULE---", in: hostNetworking),
            dnsServers: hostNetworking
                .split(separator: "\n")
                .compactMap { line in
                    let value = line.trimmingCharacters(in: .whitespaces)
                    guard value.hasPrefix("nameserver ") else { return nil }
                    return String(value.dropFirst("nameserver ".count))
                },
            searchDomains: parseSearchDomains(from: hostNetworking)
        )
        return NetworkPreservationSnapshot(
            accountNumber: device.accountNumber,
            device: device,
            summary: summary,
            captures: [
                CommandCapture(label: "host-networking", executable: "legacy-capture", arguments: [], stdout: hostNetworking, stderr: "", exitCode: 0),
                CommandCapture(label: "network-topology", executable: "legacy-capture", arguments: [], stdout: topology, stderr: "", exitCode: 0),
            ],
            directory: directory.path
        )
    }

    private func loadLegacyDevice(from url: URL) throws -> DiscoveredDevice {
        let data = try Data(contentsOf: url)
        return try JSONDecoder().decode(DiscoveredDevice.self, from: data)
    }

    private func readLegacyText(from url: URL) -> String {
...
private func shortHostName(_ value: String) -> String {
    ...
    return short.isEmpty ? "tds-helper" : short
}
