        ...
        Form {
            ...
            Section("Talos Provisioning") {
                ...
                TextField("Deployer registry address CIDR", text: $controller.settings.talos.provisioning.deployerRegistryAddressCIDR)
                    .fieldHelp("Optional node-facing IP/CIDR that tds should add to the deployer for Talos installer pulls, for example a /32 alias on the cluster data bridge.")
                TextField("Deployer registry interface", text: $controller.settings.talos.provisioning.deployerRegistryInterface)
                    .fieldHelp("Interface on the Ubuntu deployer where the registry address CIDR should be assigned, such as a bridge or VLAN reachable from Talos nodes.")
                TextField("Deployer node route interface", text: $controller.settings.talos.provisioning.deployerNodeRouteInterface)
                    .fieldHelp("Optional interface tds should force for reaching Talos node management IPs. Leave blank to use normal deployer OS routing.")
                TextField("Deployer node route source CIDR", text: $controller.settings.talos.provisioning.deployerNodeRouteSourceCIDR)
                    .fieldHelp("Optional source IP/CIDR for forced Talos node management routes. Only used when a node route interface is set.")
