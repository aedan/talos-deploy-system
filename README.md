...
- Management-network-only rendering can be enabled to simplify first cluster bring-up. In that mode, final Talos machine configs keep the static management IP/default route/DNS but omit additional bridge and VLAN interface sections until the base cluster is healthy.
