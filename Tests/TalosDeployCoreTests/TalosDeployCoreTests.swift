...
final class TalosDeployCoreTests: XCTestCase {
    ...
    func testStageWritesMaintenanceBundleForDeployerOwnedOperations() async throws {
        ...
        let metaModeBranch = try XCTUnwrap(prepareScript.range(of: "if [ \"$boot_mode\" = \"meta\" ]; then"))
        let configModeBranch = try XCTUnwrap(prepareScript.range(of: "else", range: metaModeBranch.upperBound..<prepareScript.endIndex))
        let configMap = try XCTUnwrap(prepareScript.range(of: "-map \"boot-machine-configs/${name}.yaml\" /config.yaml"))
        XCTAssertGreaterThanOrEqual(configMap.lowerBound, configModeBranch.upperBound)
        XCTAssertTrue(prepareScript.contains("|meta"))
        XCTAssertFalse(prepareScript.contains("talos.config=metal-iso talos.environment=INSTALLER_META_BASE64"))
