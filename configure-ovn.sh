...
for NODE in $ANNOTATED_NODES; do
  ...
  for FULL_BRIDGE in "${BRIDGE_ARRAY[@]}"; do
    ...
    # Strip br- prefix: KubeOVN adds it when creating bridge
    # To create bridge "br-ex", ProviderNetwork must be named "ex"
    STRIPPED_NAME="${FULL_BRIDGE#br-}"

    if [ "$STRIPPED_NAME" = "$FULL_BRIDGE" ]; then
      warn "      Bridge $FULL_BRIDGE doesn't start with 'br-' prefix"
      PROVIDER_NETWORK_NAME="$FULL_BRIDGE"
    else
      PROVIDER_NETWORK_NAME="$STRIPPED_NAME"
    fi

    PN_KEY="${PROVIDER_NETWORK_NAME}|${INTERFACE}|${FULL_BRIDGE}|${PROVIDER_NET_NAME}"
    ...
    log "      ✓ ProviderNetwork '$PROVIDER_NETWORK_NAME' will create bridge '$FULL_BRIDGE' (physnet: $PROVIDER_NET_NAME, interface: $INTERFACE)"
  done
...
for PN_KEY in "${!PROVIDER_NETWORKS[@]}"; do
  IFS='|' read -r PN_NAME INTERFACE BRIDGE PHYSNET <<< "$PN_KEY"
  log "    - ProviderNetwork '$PN_NAME' → bridge '$BRIDGE' (physnet: $PHYSNET, interface: $INTERFACE)"
done
...
for PN_KEY in "${!PROVIDER_NETWORKS[@]}"; do
  ...
  log ""
  log "ProviderNetwork: $PN_NAME"
  log "  Will create bridge: $BRIDGE"
  log "  Physical Network (OpenStack): $PHYSNET"
  log "  Interface: $INTERFACE"
  log "  Nodes: $NODES"

  if kubectl get provider-networks "$PN_NAME" &>/dev/null; then
    ACTION="updated"
    UPDATED=$((UPDATED + 1))
  else
    ACTION="created"
...
EOF
  ...
  if echo "$YAML_CONTENT" | kubectl apply -f - >> "$LOG_FILE" 2>&1; then
    log "  ✓ ProviderNetwork '$PN_NAME' $ACTION (will create bridge: $BRIDGE, physnet: $PHYSNET)"
  else
    error "  ✗ Failed to configure ProviderNetwork '$PN_NAME'"
    echo ""
...
UNIQUE_PN_NAMES=$(for PN_KEY in "${!PROVIDER_NETWORKS[@]}"; do
  IFS='|' read -r PN_NAME _ _ _ <<< "$PN_KEY"
  echo "$PN_NAME"
done | sort -u)

for PN_NAME in $UNIQUE_PN_NAMES; do
  log "  # View details for ProviderNetwork $PN_NAME"
